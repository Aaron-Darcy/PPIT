...
mysql.createPool({
    ...
    function getUser(User, Password) {
        return new Promise((resolve, reject)=>{ 
            var MyQuery = {
                //compares encrypted password with encrypted user input
    ...
    //inserts ID number of the current user logged in
    function currentlogin(User) {
     ...
     //gets user ID number so the correct details are pulled up when user wants to access them
     function Currentlogin2() {
       ...
       function logout() {
        return new Promise((resolve, reject)=>{ 
            var MyQuery = {
                sql: `delete from CurUsers`, //removes the ID number from current user so a new user can log in and access their details
            }
            pool.query(MyQuery)
            .then((data)=> {
                resolve(data)
    ...
    function addUser(User, Password){
        return new Promise((resolve, reject)=>{
            var MyQuery = {
                //encrypts password for security reasons
    ...
    function addDiet2() {
        return new Promise((resolve, reject)=>{
            var MyQuery = {
            //new details have ID number set to null, this sets ID number to current user's number
            sql: (`update IGNORE DietPlan DP, curUsers CU set DP.IDno = CU.IDno where DP.IDno is null;`),
            }
            pool.query(MyQuery)
            .then((data)=> {
               resolve(data)
    ...
    function ViewDays(IDno) {
     ...
     //sets ID number from zero to current user's ID number
     function newDays2() {
        return new Promise((resolve, reject)=>{
            var MyQuery = {
    ...
    //inserts current day values and saves details in ID number 0 to temporarly hold detail values
    function SetCurDay( Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday) {
     ...
     //removes previous details to save new details inserted by the user  
     function SetCurDay2() {
        return new Promise((resolve, reject)=>{
            var MyQuery = {
     ...
     // changes ID number from 0 to current user ID
     function SetCurDay3() {
        return new Promise((resolve, reject)=>{
            var MyQuery = {
    ...
    function addExercise(exercise){
     ...
     function addExercise2(){
        return new Promise((resolve, reject)=>{
            var MyQuery = {
                //sets ID number to current user ID
                sql: `update IGNORE TrainingPlan TP, curUsers CU set TP.IDno = CU.IDno where TP.IDno is null;`
    ...
    module.exports = {UpdatePlanTables, AllExercise, getUser, currentlogin, Currentlogin2, logout, addDiet, addDiet2, showAllDiet, SetCurDay, SetCurDay2, SetCurDay3, deleteDiet, deletePlan, ViewPlan, addUser, ViewDays, newDays, newDays2, ViewDiet, ResetDay, ResetDay2, ResetDay3, SearchExercise, addExercise, addExercise2}
